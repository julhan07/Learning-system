...
function TableList() {
    ...
    let myAccess = actions.StatusAccess(actions.me, "Pengguna");

    useEffect(() => {
        GetList();
    }, []);

    const GetList = async (url = "/api/user/list") => {
        setLoading(true);
    ...
    const confirmDelete = async () => {
        const res = await actions.Delete(`/api/user/${idSelected}`);
    ...
    return (
        <Fragment>
            <BreadCrumb
                ...
                active_label="Table"
            />
            <Modal id="delete-user" title="Konfirmasi">
            ...
            <div className="card">
                <div className="card-header">
                    <div className="row">
                        <div className="col-md-8">Data Pengguna</div>
                        <div className="col-md-4">
                            {myAccess[0] &&
                                myAccess[0].indexOf("create") !== -1 && (
                                    <Link
                                        to="/dashboard/user?action=add"
                                        style={{ float: "right" }}
                                                    ...
                                                    {myAccess[0] &&
                                                        myAccess[0].indexOf(
                                                            ...
                                                            <Link
                                                                ...
                                                                to={`/dashboard/user?action=edit&id=${item.id}`}
                                                            >
                                                                Ubah
                                                            </Link>
                                                        )}
